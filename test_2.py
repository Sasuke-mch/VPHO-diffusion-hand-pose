...
class FeatureRefinement(nn.Module):
    ...
    def __init__(self, in_channels=2048):
        super().__init__()
        # 两个瓶颈块，输入输出通道均为 2048（因为 expansion=4，planes=512 时输出 2048）
...
if __name__ == "__main__":
    ...
    print("训练完成！")
    plot_loss_curve(batch_losses, "train_loss_curve_batch_test_2.png")

    image_bgr = cv2.imread("train_loss_curve_batch_test_2.png")

    # 显示真实图像
    cv2.imshow("loss curve of test 2", image_bgr)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    torch.save(model.state_dict(), "diffusion_model_with_feature_refinement.pth")
    print("模型已保存到 diffusion_model_with_feature_refinement.pth")

    # 加载训练好的权重
    model_path = "diffusion_model_with_feature_refinement.pth"
