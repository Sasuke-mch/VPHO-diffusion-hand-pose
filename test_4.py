...
class ConditionalDiffusionModel(nn.Module):
    ...
    def forward(self, theta_t, t, psi, beta):
        """
        Args:
            theta_t: [B, pose_dim] 带噪姿态
            t: [B] 时间步
            psi: [B, cond_dim] 图像特征
            beta: [B, beta_dim] 形状参数
        Returns:
            eps_pred: [B, pose_dim] 预测的噪声
        """
        # 时间嵌入
...
def plot_loss_curve(losses: list, save_path: str = "train_loss_curve.png"):
    ...
    # 删除了 marker='o'，因为按 batch 记录点数太多，图表会看不清
    plt.plot(range(1, len(losses) + 1), losses, linestyle='-', color='b', label='Train Loss')
    plt.title("Training Loss Over Batches without fr")
...
if __name__ == "__main__":
    ...
    feature_extractor = nn.Sequential(
        *list (feature_extractor.children())[:-2],  # 去掉最后的全局池化和分类层
    ...
    print("训练完成！")
    plot_loss_curve(batch_losses, "train_loss_curve_batch_test_4.png")

    image_bgr = cv2.imread("train_loss_curve_batch_test_4.png")

    # 显示真实图像
    cv2.imshow("loss curve of test 4", image_bgr)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    torch.save(model.state_dict(), "diffusion_model_with_vpho_conditional_diffusion_without_fr.pth")
    print("模型已保存到 diffusion_model_with_vpho_conditional_diffusion_without_fr.pth")

    # 加载训练好的权重
    model_path = "diffusion_model_with_vpho_conditional_diffusion_without_fr.pth"
