...
if __name__ == "__main__":
    ...
    print("训练完成！")
    plot_loss_curve(batch_losses, "train_loss_curve_batch_test_3.png")

    image_bgr = cv2.imread("train_loss_curve_batch_test_3.png")

    # 显示真实图像
    cv2.imshow("loss curve of test 3", image_bgr)
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    torch.save(model.state_dict(), "diffusion_model_with_vpho_conditional_diffusion.pth")
    print("模型已保存到 diffusion_model_with_vpho_conditional_diffusion.pth")

    # 加载训练好的权重
    model_path = "diffusion_model_with_vpho_conditional_diffusion.pth"
